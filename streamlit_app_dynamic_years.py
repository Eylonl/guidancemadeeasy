        ...
        if low_val < 0 and high_val > 0:
            ...
            if should_both_be_negative:
                ...
                if isinstance(high, str):
                    ...
                    elif 'continue  # Skip if we can't convert to float
        else:
            high_val = high
        ...
        # 2. If low is positive and high is negative, this is likely an error too
            ...
            if 'Average' in df.columns and not pd.isnull(row.get('Average')):
                ...
                if isinstance(row.get('Average'), str):
                    ...
                    df.at[idx, 'Average'] = avg in row.get('Average', ''):
                            if abs(avg) >= 100:
                                df.at[idx, 'Average'] = f"${avg:.0f}"
                            elif abs(avg) >= 10:
                                df.at[idx, 'Average'] = f"${avg:.1f}"
        ...
        # 2. If low is positive and high is negative, this is likely an error too
            ...
            if 'Average' in df.columns and not pd.isnull(row.get('Average')):
                ...
                if isinstance(row.get('Average'), str):
                    ...
                    df.at[idx, 'Average'] = avg in df.loc[idx, col]:
                            if abs(val) >= 100:
        ...
        # 2. If low is positive and high is negative, this is likely an error too
            ...
            if 'Average' in df.columns and not pd.isnull(row.get('Average')):
                ...
                if isinstance(row.get('Average'), str):
                    ...
                    df.at[idx, 'Average'] = avg in original_value and 
                not '(continue  # Skip if we can't convert to float
        else:
            high_val = high
...
def split_gaap_non_gaap(df):
    ...
    return pd.DataFrame(rows)continue  # Skip if we can't convert to float
        else:
            high_val = high
